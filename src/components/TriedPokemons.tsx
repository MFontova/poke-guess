    ...
    <section className="flex items-start gap-8 overflow-y-auto max-w-full">
      {
        reversedTries.map((pokemon) => (
          <div key={pokemon.id} className="flex flex-col gap-3 items-center">
            ...
            <Stat compareValue={selectedPokemon!.types[1] ? selectedPokemon!.types[1].type.name : selectedPokemon!.types[0].type.name} title="Tipo 2" value={pokemon.types[1] ? pokemon.types[1].type.name : pokemon.types[0].type.name} />
